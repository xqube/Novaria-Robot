...
export async function saveUserInfo(ctx) {
  try {
    ...
    if (chatDetails.type === "private") {
      ...
      if (existingUser) {
        ...
        const infoObject = new UserModel({
          id: id || null,
          fname: fname || null,
          lname: lname || null,
        ...
        if (id) infoObject.id = id;
        if (fname) infoObject.fname = fname;
        if (lname) infoObject.lname = lname;
        if (username) infoObject.username = username;
